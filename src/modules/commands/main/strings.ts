...
export default
{
    ...
    execute(session:Session, user_input:UserInput, local_data:LocalData)
    {
        ...
        if(!file){ return `Unable to find file with name "${FILE_NAME}"` }
        if(!(file instanceof TextFile)){ return new InvalidFileTypeError(file.name, "text") }

        if(!flag){ if(file.contents){ return new List().add(file.contents) } else { return 1 } }
        switch(flag)
        {
            case "-e":
            case "--edit":
                if(!user_input.recieved(1)){ return "Not enough arguments provided" }
                ...
                switch(second_argument)
                {
                    case "-a":
                    case "--add":
                        if(file.contents){ file.contents += user_input.toString() }
                        else{ file.contents = user_input.toString() }
