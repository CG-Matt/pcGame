...
export default {
    ...
    execute(session, user_input, local_data) {
        ...
        if (typeof destination[1] !== "string") {
            return 1;
        }
        ...
        if (!target) {
            return `ERROR: No machine exists with that name and IP address`;
        }
        if (!(target instanceof Computer)) {
            return new InvalidDeviceError();
        }
        if (target.ports.getByID(target_port).isOpen()) {
            return `ERROR: Machine has no SSH listener or its ssh port is closed`;
        }
        if (target.ports.getByID(target_port).name !== "ssh") {
            return 1;
        }
        new UI(session, {
