...
export default {
    ...
    execute(session, user_input, local_data) {
        ...
        if (!flag) {
            ...
            let target_file_name, target_file_extension;
            ...
            if (!exist) {
                ...
                if (!argument) {
                    return `No target file name provided use "import <file_name> as <target_file_name>" to specify a name or use "import <file_name>" to import as current name`;
                }
                const { file_name: t_file_name, file_extension: t_file_extension, error } = splitName(argument);
                if (error) {
                    target_file_name = argument;
                }
                else {
            ...
            if (!readdirSync("imports/").includes(full_file_name)) {
                return `Unable to find a file with the name "${full_file_name}", use "import -ls" to list all files`;
            }
