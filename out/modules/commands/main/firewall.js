...
import { Reply } from "../../internal/classes/executeTypes.js";
            ...
            switch (user_input.first()) {
                ...
                case "-o":
                    if (!user_input.recieved(1)) {
                        return new Reply("Please provide a port name", "ERR");
                    }
                    var port = client.ports.getByName(user_input.array[1]);
                    if (!port) {
                        return new Reply(`No port with name ${user_input.array[1]} found`, "ERR");
                    }
                    port.open();
