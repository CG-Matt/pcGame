...
export default {
    ...
    execute(session, user_input, local_data) {
        ...
        if (!website) {
            return `Unable to resolve domain "${domain}"`;
        }
        if (!flag) {
            user_input.shift();
        ...
        switch (flag) {
            ...
            case "--download":
                ...
                if (!exist) {
                    return `File to download not specified please use "-f <file_name>" to specify a file to download`;
                }
                if (!file_name) {
                    return `No file name provided please specify it with "-f <file_name>"`;
                }
                if (!file) {
                    return `The domain "${website.name}" does not contain a file with the name "${file_name}"`;
                }
...
function display_website_data(website) {
    return new List()
        .importArray(website.listFiles())
        .addFirst(Header("<FILES>", 60))
        .add(Footer(60));
}
function check_files(website) {
    if (!website.hasFiles()) {
        return `The domain "${website.name}" does not contain any files`;
    }
    if (!website.allowGuestFileAccess) {
        return `Access to files in not permitted`;
    }
}
