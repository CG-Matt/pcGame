...
export default {
    ...
    execute(session, user_input, local_data) {
        ...
        if (!ip_provided) {
            return `IP address to query was not specified, use "-i <ip_address>" to specify it`;
        }
        if (!port_provided) {
            return `Port to query was not specified, use "-p <port>" to specify it`;
        }
        if (!testIsIP(ip_address)) {
            return `"${ip_address}" is not recognised as a valid address`;
        }
    }
};
